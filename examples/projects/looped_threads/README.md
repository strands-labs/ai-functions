...
```bash
cd examples/projects/looped_threads
uv run main.py
```
...
(`scikit-learn`, `matplotlib`, `pandas`, plus `strands-ai-functions` and the shared
`example_helpers` package from `ai-functions-examples`),
