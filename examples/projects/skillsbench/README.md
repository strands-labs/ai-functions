...
2. **Python dependencies** — from the repository root:
   ...
   (or `pip install -e ".[claude-code]" -e examples pytest openpyxl pandas python-docx
...
4. **Credentials** — the agent runs through the Claude Code CLI
   ...
   Bedrock through the shared cached `models.medium` preset, so AWS credentials with
   Bedrock access must be present in the environment.

## Usage

```bash
uv run --package ai-functions-skillsbench-example \
    examples/projects/skillsbench/memory_improvement_loop.py \
