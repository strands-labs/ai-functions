...
from example_helpers import economics, models
...
from ai_functions.experimental.economics.beliefs import forecast
...
# forecaster something. Swap in a real search tool (see example_helpers/utils.py's
...
memory = JSONMemoryBackend(
    schema=ForecastMemory, actor_id="economics-demo", path="./forecast_memory.json", model=economics.sonnet.model
)
...
@routed(
    models=[economics.haiku, economics.sonnet],
    value=0.05,  # good sources are worth 5 cents
    beliefs=LLMForecaster(
        forecast_fn=forecast.replace(model=models.medium), memory=memory, memory_key="researcher_routing"
    ),
    budget=0.10,
    policy=Greedy(),
)
...
@routed(
    ...
    beliefs=LLMForecaster(
        forecast_fn=forecast.replace(model=models.medium), memory=memory, memory_key="writer_routing"
    ),
    budget=0.15,
    policy=Greedy(),
)
