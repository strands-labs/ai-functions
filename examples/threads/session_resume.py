...
re-invoking the script::
    ...
    cd examples && uv run threads/session_resume.py --session-dir /tmp/ai_functions-demo
...
# leaves these ``None`` (the template uses ``models.medium``) and the test passes a
...
async def _run_one_thread(
    ...
    *,
    ...
    Args:
        ...
        model: Optional ``strands.Model`` to inject; ``None`` keeps the template's model.
    ...
    *,
    ...
    Args:
        ...
        models: Optional mapping of thread name to a ``strands.Model`` to
            ...
            ``None``, each agent uses ``models.medium``.
