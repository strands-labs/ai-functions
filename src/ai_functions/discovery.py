...
import platformdirs  # pyright: ignore[reportMissingTypeStubs]
from pydantic import BaseModel, ValidationError

_RUNTIME_FILENAME = "coordinator.json"
_ENV_URL_OVERRIDE = "AI_FUNCTIONS_COORDINATOR_URL"
...
class CoordinatorAlreadyRunningError(RuntimeError):
    ...
    exists and its PID is alive. The CLI surfaces this as "an ai_functions
...
def runtime_dir() -> Path:
    ...
    Returns:
        ...
        exist on disk.
    """
    # ``user_runtime_dir`` is the XDG runtime dir on Linux; platformdirs falls
    # back to a user state dir on other platforms. Using it directly is fine;
    # we only force the fallback if platformdirs raises.
    try:
        base: str = platformdirs.user_runtime_dir("ai_functions", ensure_exists=False)
    except Exception:
        base = platformdirs.user_state_dir("ai_functions", ensure_exists=False)
        return Path(base) / "run"
    return Path(base)
...
def runtime_file_path() -> Path:
    ...
    """
    return runtime_dir() / _RUNTIME_FILENAME
...
def _pid_alive(pid: int) -> bool:
    """Return True if ``pid`` refers to a process we could signal."""
    if pid <= 0:
        return False
    try:
        os.kill(pid, 0)
        ...
        # Windows: errno.ESRCH for "no such process" under os.kill emulation.
...
def _read_file(path: Path) -> RuntimeInfo | None:
    """Parse the runtime file at ``path``; return None on any failure."""
    try:
        ...
        return None
    try:
        return RuntimeInfo.model_validate_json(raw)
...
def write_runtime_info(info: RuntimeInfo) -> Path:
    ...
    if existing is not None and _pid_alive(existing.pid):
        raise CoordinatorAlreadyRunningError(
            f"an ai_functions coordinator is already advertised at {path} (pid={existing.pid}, url={existing.url})",
        )
    if existing is not None or path.exists():
        # Stale or unparseable; clear it before writing.
        try:
            path.unlink()
        except FileNotFoundError:
            pass
    ...
    try:
        ...
        # Best-effort cleanup of the tmp file if we never renamed it.
        try:
            os.unlink(tmp_path)
        except FileNotFoundError:
            pass
...
def read_runtime_info() -> RuntimeInfo | None:
    ...
    if info is None:
        if path.exists():
            # Corrupt / unparseable file; remove it.
            try:
                path.unlink()
            except FileNotFoundError:
                pass
...
def discover_coordinator() -> RuntimeInfo:
    ...
    """
    override = os.environ.get(_ENV_URL_OVERRIDE)
    if override:
        return RuntimeInfo(
            url=override,
            pid=0,
    ...
    if info is None:
        raise NoCoordinatorError(
            "no ai-functions coordinator is advertised on this host; "
            "start one with 'ai-functions server' or set "
            f"{_ENV_URL_OVERRIDE} to an existing coordinator URL",
        )
    return info
